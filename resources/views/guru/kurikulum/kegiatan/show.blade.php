<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Detail Laporan Kegiatan Kurikulum') }}
        </h2>
    </x-slot>
                    ...
                    <div class="flex justify-between items-center mt-8 pt-6 border-t border-gray-100">
                        ...
                        <div class="flex gap-3">
                            ...
                            @if (Auth::user()->role == 'admin' || (Auth::id() == $kegiatan->user_id && $kegiatan->status == 'pending'))
                                <a href="{{ route('kurikulum.kegiatan.edit', $kegiatan->id) }}"
                                    class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-5 rounded shadow transition">
                                    Edit Data
                                </a>

                                <form action="{{ route('kurikulum.kegiatan.destroy', $kegiatan->id) }}" method="POST"
