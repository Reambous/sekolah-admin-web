<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Edit Kegiatan Humas') }}</h2>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('humas.kegiatan.update', $kegiatan->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        {{-- 2. TEXTAREA YANG BERSIH --}}
                        <div class="mb-6">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Refleksi / Keterangan</label>
                            <textarea name="refleksi" rows="4"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>{{ $kegiatan->refleksi }}</textarea>
                        </div>
